"""OPERATOR-ONLY: change one listing directly in the database, bypassing wallet
signatures. For cases the signed API cannot cover - e.g. re-pointing submitted_by after
the owner wallet is lost - never for routine edits (use the signed PATCH for those).
...
def _run(args: argparse.Namespace) -> int:
    ...
    with psycopg.connect(url, row_factory=dict_row) as conn:
        ...
        guard = " AND ".join(f"{f} IS NOT DISTINCT FROM %(_exp_{f})s" for f in expected)
        for f in expected:
